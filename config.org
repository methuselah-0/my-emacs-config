...
First install the python-pyenv package on guix. Then:
...
finally, setup a =.dir-locals.el= file in the project root as well,
with this content, making sure that the =venv= is recognized by
flycheck:
