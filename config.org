...
;; (global-set-key [(meta shift j)]  'windmove-left)
;; (global-set-key [(meta shift l)]  'windmove-right)
;; (global-set-key [(meta shift i)]  'windmove-up)
;; (global-set-key [(meta shift k)]  'windmove-down)
...
#+emacs-lisp
(add-to-list 'load-path "/home/user1/src/my-emacs-config/emacs-webkit")
(module-load "/home/user1/src/my-emacs-config/emacs-webkit/webkit-module.so")
(require 'webkit)
...
remember you can use ~M-x list-flycheck-errors~ to get the errors
described in the minibuffer in case you have the
flycheck-indication-mode set to left-margin and that conflicts with
diff-hl-margin-mode's error indication in the same place.
